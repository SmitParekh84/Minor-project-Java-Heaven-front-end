...
const Cart = () => {
  ...
  const [error, setError] = useState(""); // Error state for checkout
        ...
        <div className="container mx-auto p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cartItems.map((item, index) => (
              <div key={index} className="flex flex-col justify-between p-4 bg-white shadow-lg rounded-lg">
                <div>
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  <p className="text-gray-700 mt-2 font-semibold">Subtotal: ₹ {(item.price * item.quantity).toFixed(2)}</p>
                </div>
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center">
                    <button
                      onClick={() => handleDecreaseQuantity(item.id, item.size)}
                      className="bg-gray-200 text-gray-800 px-2 py-1 rounded-lg transition-colors duration-300 hover:bg-gray-300"
                    >
                      -
                    </button>
          ...
          <form onSubmit={handleCheckout} className="mt-8">
            ...
            <input
              ...
              required
              className="border border-gray-300 p-2 mb-2 w-full rounded transition duration-200 focus:ring focus:ring-blue-400"
            />
            <input
              type="email"
              name="email"
            ...
            <button
              ...
              className={`bg-green-500 text-white py-2 px-6 rounded-lg mt-4 hover:bg-green-600 transition ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
            >
